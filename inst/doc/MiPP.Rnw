...
Good classifying genes may be masked by other better classifying genes, 
so the genes may be discovered if the other genes are not present.  
Therefore, it is worth selecting gene models after removing genes selected in the first run.
The \Rpackage{MiPP} package also enables to sequentailly select gene models after removing genes selected in the previous runs.

For the acute leukemia data, the sequntial analysis can be performed by the following arguments in the \Rfunction{mipp.seq} function: 
The argument \Rfunarg{n.seq=3} means that the sequentaik analysis is performed 3 times. 
...
For the colon cancer data, the sequntial analysis can be performed by the following arguments: 
